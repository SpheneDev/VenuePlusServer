...
public static class PublicEndpoints
{
    ...
    {
        app.MapGet("/health", () => Results.Ok(new { ok = true, time = DateTimeOffset.UtcNow })).RequireCors("PublicJson");
        ...
        app.MapGet("/{accessKey}/viplist.json", async (string accessKey, HttpContext ctx) =>
        {
            ...
            catch (Exception ex)
            {
                ...
                return Results.Json(Array.Empty<VipEntry>());
            }
        ...
        app.MapGet("/{accessKey}/stafflist.json", async (string accessKey, HttpContext ctx) =>
        {
            ...
            try
            {
                ...
                if (!string.IsNullOrWhiteSpace(conn))
                {
                    ...
                    var users = list.OrderBy(u => u.Username, StringComparer.Ordinal).Select(u => new StaffUser { Username = u.Username, Job = u.Job, Role = u.Role, CreatedAt = u.CreatedAt, Uid = u.Uid }).ToArray();
                ...
                else
                {
                    ...
                    var users = usersClub.Select(u => new StaffUser
                    {
                        Username = u,
                        ...
                        Uid = (Store.StaffUsers.TryGetValue(u, out var info3) ? info3.Uid : u)
                    }).ToArray();
            ...
            catch (Exception ex)
            {
                ...
                return Results.Json(Array.Empty<StaffUser>());
            }
        }).RequireCors("PublicJson");
